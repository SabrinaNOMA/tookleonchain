...
<body>
  ...
  <div class="wrap">
    <div class="card">
      ...
      <button class="close-btn" onclick="closeFrame()" title="Close">
        &times;
      </button>
  ...
  <script>
    // Fonction pour fermer la modale parente
    function closeFrame() {
      // Si on est dans une iframe et que le parent a la fonction closeKycModal
      if (window.parent && typeof window.parent.closeKycModal === 'function') {
        window.parent.closeKycModal();
      } else {
        // Fallback : essayer de masquer l'élément par son ID si la fonction n'existe pas
        try {
            if(window.parent.document.getElementById('kycModal')) {
                window.parent.document.getElementById('kycModal').style.display = 'none';
            }
        } catch(e) {
            console.log('Cannot close modal automatically.');
        }
      }
    }
  </script>
