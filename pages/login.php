...
<body>
  <div class="auth-card">
    ...
    <form id="auth-form" novalidate>
      ...
      <!-- Terms & Privacy consent (REGISTER only) -->
      <div id="terms-section" class="mb-3" style="display:none;">
        <label class="flex items-start gap-2 text-sm">
          <input type="checkbox" id="accept_terms" name="accept_terms" value="1" class="mt-1">
          <span>
            I agree to the <a href="/pages/terms.php" target="_blank" class="font-medium text-[var(--primary-purple)]">Terms of Service</a>
            and <a href="/pages/privacy.php" target="_blank" class="font-medium text-[var(--primary-purple)]">Privacy Policy</a>.
          </span>
        </label>
      </div>
  ...
  <script>
    ...
    (function(){
      ...
      switchLinkEl.addEventListener('click', (e)=>{
        ...
        if (isLoginMode){ submitButton.textContent='Login'; nameSection.style.display='none'; repeatSection.style.display='none'; if(termsSection) termsSection.style.display='none'; }
        else { submitButton.textContent='Register'; nameSection.style.display='block'; repeatSection.style.display='block'; if(termsSection) termsSection.style.display='block'; }
      });
    })();
    ...
    document.getElementById('auth-form').addEventListener('submit', async (e) => {
      ...
      // Terms & Privacy consent required on Register
      if (!isLoginMode) {
        const cb = document.getElementById('accept_terms');
        if (!cb || !cb.checked) { showMsg('Please accept the Terms of Service and Privacy Policy to continue.'); return; }
      }
