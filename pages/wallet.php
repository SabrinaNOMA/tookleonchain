...
<style>
    ...
    #branded-modal-input:focus { border-color: #4f46e5; box-shadow: 0 0 0 4px rgba(79, 70, 229, 0.1); outline: none; }
                ...
                <div id="existing-wallet-card" class="simple-card p-6" style="<?php echo empty($existingCoinbaseWallet) ? 'display:none;' : ''; ?>">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 bg-gray-50 rounded-lg border border-gray-100">
                            ...
                            <!-- ✅ AJOUT/ALIGN PROD : bouton export -->
                        ...
                        <!-- ✅ AJOUT/ALIGN PROD : bouton suppression -->
                    ...
                    <div class="simple-card p-6 bg-indigo-50/20 border border-indigo-100 shadow-sm">
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                ...
                                <p class="text-xs text-indigo-800/70">Create a new secure embedded wallet using just your email.</p>
                            </div>
                        </div>
        ...
        <div class="mt-12 pt-8 border-t border-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white border border-gray-200 rounded-lg p-5 shadow-sm hover:shadow-md transition-all">
                    <h4 class="font-bold text-gray-900 text-xs mb-3 flex items-center gap-2"><i data-lucide="wallet" class="w-4 h-4 text-gray-600"></i>Digital Wallet</h4>
                    <p class="text-xs text-gray-500 leading-relaxed">A digital tool that lets you interact with the blockchain.</p>
                </div>
                <div class="bg-white border border-gray-200 rounded-lg p-5 shadow-sm hover:shadow-md transition-all">
                    <h4 class="font-bold text-gray-900 text-xs mb-3 flex items-center gap-2"><i data-lucide="zap" class="w-4 h-4 text-indigo-600"></i>Embedded Wallet</h4>
                    <p class="text-xs text-gray-500 leading-relaxed">A smart wallet integrated directly into your account.</p>
                </div>
                <div class="bg-white border border-gray-200 rounded-lg p-5 shadow-sm hover:shadow-md transition-all">
                    <h4 class="font-bold text-gray-900 text-xs mb-3 flex items-center gap-2"><i data-lucide="key" class="w-4 h-4 text-gray-600"></i>Private Keys & MPC</h4>
                    <p class="text-xs text-gray-500 leading-relaxed">Private keys are the "password" to your funds.</p>
                </div>
            </div>
        </div>
...
<!-- ✅ AJOUT/ALIGN PROD : MODAL EXPORT (IFRAME) -->
<div id="export-modal" class="modal-overlay">
    <div class="modal-content relative bg-white h-[85vh] flex flex-col">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-white">
            <div class="flex items-center gap-2">
                <i data-lucide="key" class="w-5 h-5 text-indigo-600"></i>
                <h3 class="text-base font-bold text-gray-800">Export Private Key</h3>
            </div>
            <button class="text-gray-400 hover:text-gray-600 close-export-btn">
                <i data-lucide="x" class="w-5 h-5"></i>
            </button>
        </div>

        <div class="flex-1 bg-gray-50 relative">
            <iframe id="export-iframe" src="" class="w-full h-full border-0 absolute inset-0" allow="clipboard-read; clipboard-write"></iframe>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // PROCEED TO IFRAME (WITH LOADING OVERLAY)
    if(proceedBtn) {
    ...
    function showModal(message, isError = null) {
        ...
        if (isError === null) {
             ...
             isError = (lower.includes('error') || lower.includes('invalid') || lower.includes('failed'));
        }
    ...
    window.alert = function(message) { showModal(message, null); };
        ...
        const observer = new MutationObserver(function(mutations) {
            ...
            if (match && match[0]) {
                ...
                if(manualInput.value === "") {
                    ...
                    manualInput.style.backgroundColor = "#f0fdf4";
                    setTimeout(() => { manualInput.scrollIntoView({ behavior: 'smooth', block: 'center' }); manualInput.focus(); }, 500);
                }
            }
        });
    ...
    if (manualSaveBtn) {
        manualSaveBtn.addEventListener('click', function(e) {
            ...
            if (!address || !address.startsWith('0x') || address.length < 40) {
                showModal("Invalid address. Please copy the 0x... address.", true);
                return;
            }
    ...
    if(deleteBtn) {
        deleteBtn.addEventListener('click', (e) => {
             ...
             document.getElementById('branded-modal-confirm-actions').style.display = 'none';
             document.getElementById('branded-modal-delete-actions').style.display = 'flex';

             title.textContent = "Are you sure?";
             msg.innerHTML = "You are about to disconnect this wallet. <br><b>Make sure you have backed up your key.</b>";
             ...
             newConfirm.addEventListener('click', (ev) => {
                ...
                .then(r => r.ok ? r.json() : Promise.reject("Backend not found"))
                .then(d => {
                    modal.classList.remove('show');
                ...
                .catch(err => {
                    ...
                    existingCard.style.display = 'none';
                    createSection.style.display = 'block';
                    showModal("Wallet disconnected (UI Only).", false);
                    newConfirm.textContent = "Yes, Delete";
                });
             });
        });
    ...
    // --- EXPORT/REVEAL LOGIC (IFRAME MODE) ---
    ...
    if(exportBtn) {
        exportBtn.addEventListener('click', (e) => {
             e.preventDefault();
             // ✅ URL export (aligné PROD)
             const exportUrl = '/wallet-export/cdp-export/index.html';

             // Charger l'iframe seulement si nécessaire
             const abs = new URL(exportUrl, window.location.origin).toString();
             if(exportIframe.src !== abs) {
                 exportIframe.src = abs;
             }
