...
// Tu peux aussi récupérer une adresse embedded Tookle, par ex :
// $prefillWallet = $embedded_wallet_address ?? '';
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<body>
  <div class="container mt-5">
    <div class="card shadow-sm p-4">
      <h4 class="mb-3 text-center">💶 Onramp Tookle – Session Token</h4>

      <div class="mb-3">
        <label class="form-label">Adresse du wallet destinataire</label>
        <input
          type="text"
          class="form-control"
          id="wallet_address"
      ...
      <div class="mb-3">
        <label class="form-label">Stablecoin à recevoir</label>
      ...
      <div class="mb-3">
        <label class="form-label">Réseau blockchain</label>
  ...
  document.getElementById('startBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        resultDiv.textContent = 'Erreur HTTP ' + res.status + ' : ' + (data.error || text);
      ...
      if (data.success && data.session_url) {
        ...
        resultDiv.innerHTML =
          '✅ Session créée.<br><a href="' +
          data.session_url +
          '" target="_blank" class="btn btn-success mt-2">Ouvrir Coinbase Onramp</a>';
        resultDiv.classList.remove('d-none');
        return;
      }
      ...
      resultDiv.className = 'alert alert-danger';
      resultDiv.textContent = 'Erreur JS : ' + err.message;
      resultDiv.classList.remove('d-none');
    }
  });
  </script>
